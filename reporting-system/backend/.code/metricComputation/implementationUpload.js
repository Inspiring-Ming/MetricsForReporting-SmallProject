...
exports.saveAndCompileUserPythonScript = saveAndCompileUserPythonScript;
exports.executeSavedUserPythonScriptFlexible = executeSavedUserPythonScriptFlexible;
 ...
 */
async function validatePythonCode(source, language) {
    if (language !== "python")
        throw (0, http_errors_1.default)(400, "Unsupported language");
    if (typeof source !== "string")
        throw (0, http_errors_1.default)(400, "Invalid 'code' payload");
    const MAX_CODE_BYTES = 100 * 1024; // 100KB
    if (Buffer.byteLength(source, "utf8") > MAX_CODE_BYTES) {
        throw (0, http_errors_1.default)(413, "Code too large");
    }
    const snippet = `
...
`;
    ...
    try {
        const out = await python_shell_1.PythonShell.runString(snippet, {
            args: [payload],
            pythonOptions: ["-I", "-S", "-B"],
            mode: "text",
        });
        ...
        if (!last)
            throw (0, http_errors_1.default)(500, "Validator produced no output");
        return JSON.parse(last);
    }
    catch {
        throw (0, http_errors_1.default)(500, "Failed to validate code");
    }
}
/**
 ...
 */
async function saveAndCompileUserPythonScript(source, language, name) {
    const validation = await validatePythonCode(source, language);
    if (!validation.ok) {
        return { ok: false, error: validation.error, errors: validation.errors };
    }
    const id = crypto_1.default.createHash("sha256").update(source, "utf8").digest("hex");
    const baseDir = path_1.default.join(process.cwd(), "models", "user_scripts");
    ensureDir(baseDir);
    ...
    const pyPath = path_1.default.join(baseDir, fileName);
    const pycPath = path_1.default.join(baseDir, `${path_1.default.parse(fileName).name}.pyc`);
    const snippet = `
...
`;
    ...
    try {
        ...
        throw (0, http_errors_1.default)(500, "Failed to persist/compile code");
    }
}
/**
 ...
 */
async function executeSavedUserPythonScriptFlexible(identifier, inputs) {
    if (typeof identifier !== "string" || !identifier.trim()) {
        throw (0, http_errors_1.default)(400, "Missing script identifier");
    }
    const baseDir = path_1.default.join(process.cwd(), "models", "user_scripts");
    let pyPath;
    if (/^[a-f0-9]{64}$/.test(identifier)) {
        // Treat as content hash id
        pyPath = path_1.default.join(baseDir, `${identifier}.py`);
    }
    else {
        // Treat as user-supplied filename
        const fileName = sanitizeFilename(identifier);
        if (!fileName)
            throw (0, http_errors_1.default)(400, "Invalid script name");
        pyPath = path_1.default.join(baseDir, fileName);
    }
    if (!fs_1.default.existsSync(pyPath)) {
        throw (0, http_errors_1.default)(404, "Script not found");
    }
...
`;
    try {
        ...
        if (data.ok)
            return { ok: true, result: data.result, logs: out };
